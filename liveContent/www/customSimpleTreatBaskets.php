<?php
  ...
  if ($conn) {
     ...
     $query = "SELECT 
                 id, imageName, itemName, price, category, description 
                 FROM catalog 
                 WHERE 
                 status='active' AND
...
//        alert('onload');
      ...
      <table id='mainTable'>
        ...
        <tr id='head2'>
          <td colspan="6" style="text-align: center;"><h3>Custom Simple Treat Baskets $35</h3></td>
        </tr>
        <tr id='head3' class='bordered-row'>
        ...
        <tr id='line0' class='bordered-row'>
          <td id='cookie0'></td>
          <td id='cookieQty0'>
            <input type='number' id='cookieInput0' style='width:40px' value=0>
          </td>
          <td id='cake0'></td>
          <td id='cakeQty0'>
            <input type='number' id='cakeInput0' style='width:40px' value=0>
          </td>
          <td id='treat0'></td>
          <td id='treatQty0'>
            <input type='number' id='treatInput0' style='width:40px' value=0>
          </td>
        </tr>
        <tr id='dataHead2'>
	  <td colspan='2'>Pick 12 cookies,</td>
	  <td colspan='2'>Pick one cake flavor</td>
	  <td colspan='2'>Choose between 3 cakesicles</td>
        </tr>
        <tr id='dataHead3'>
	  <td colspan='2'>can be an assortment</td>
	  <td colspan='2'>or 1 brownie</td>
        </tr>
        <tr id='dataHead4'>
	  <td colspan="5" style="text-align: center">All baskets include 3 pretzel sticks</td>
        </tr>
        <tr id='emailLine' class='bordered-row'>
...
//        alert('addRow('+rowNbr+')');
          ...
          const cakeQty = "<input id='cakeInput"+rowNbr+"' style='width:40px' value='0' type='number'>";
          cakeQty.setAttribute('id','cakeInput'+rowNbr);
          ...
          rows.forEach((row, rowIndex) => {
            ...
            cells.forEach((cell, cellIndex) => {
              if (cell.id) {
                ...
                cellToDelete = cellIndex + 1;
                console.log(` Row ${rowIndex} ID: ${row.id} Cell ${cellIndex}: cellId`);
                if (cellText == '') {
                   row.deleteDell(cellToDelete);
                }
              } // if (cell.id)
            }); // cells.forEach((cell, cellIndex) =>
          }); // rows.forEach((row, rowIndex) =>
